...
void Reset_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)Reset_ISR__INTC_NUMBER] = address;
}
...
cyisraddress Reset_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)Reset_ISR__INTC_NUMBER];
}
...
{
    *Reset_ISR_INTC_PRIOR = priority << 5;
}
...
uint8 Reset_ISR_GetPriority(void)
{
    ...
    priority = *Reset_ISR_INTC_PRIOR >> 5;

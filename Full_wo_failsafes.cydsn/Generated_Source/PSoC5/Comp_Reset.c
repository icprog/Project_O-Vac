...
uint8 Comp_Reset_initVar = 0u;

/* Internal functions definitoin */
static void Comp_Reset_trimAdjust(uint8 nibble) ;

/* static Comp_Reset_backupStruct  Comp_Reset_backup; */
#if (CY_PSOC5A)
    static Comp_Reset_LOWPOWER_BACKUP_STRUCT  Comp_Reset_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_Reset_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_Reset_Init
********************************************************************************
*
* Summary:
...
void Comp_Reset_Init(void) 
{
    ...
    Comp_Reset_SetSpeed(Comp_Reset_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_Reset_DEFAULT_HYSTERESIS == 0u )
        Comp_Reset_CR |= Comp_Reset_HYST_OFF;
    #else
        Comp_Reset_CR &= (uint8)(~Comp_Reset_HYST_OFF);
    #endif /* Comp_Reset_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_Reset_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_Reset_CR &= (uint8)(~Comp_Reset_PWRDWN_OVRD);
        #else 
            Comp_Reset_CR |= Comp_Reset_PWRDWN_OVRD;
        #endif /* Comp_Reset_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_Reset_CR |= Comp_Reset_MX_AO;

    /* Set default sync */
    Comp_Reset_CLK &= (uint8)(~Comp_Reset_SYNCCLK_MASK);
    #if ( Comp_Reset_DEFAULT_BYPASS_SYNC == 0u )
        Comp_Reset_CLK |= Comp_Reset_SYNC_CLK_EN;
    #else
        Comp_Reset_CLK |= Comp_Reset_BYPASS_SYNC;
    #endif /* Comp_Reset_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_Reset_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_Reset_PWRMGR |= Comp_Reset_ACT_PWR_EN;
    Comp_Reset_STBY_PWRMGR |= Comp_Reset_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_Reset_restoreReg == 1u)
        {
            Comp_Reset_CR = Comp_Reset_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_Reset_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Comp_Reset_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Comp_Reset_initVar: Is modified when this function is called for the 
...
void Comp_Reset_Start(void) 
{
    ...
    if ( Comp_Reset_initVar == 0u )
    {
        Comp_Reset_Init();
        
        Comp_Reset_initVar = 1u;
    }   
...
void Comp_Reset_Stop(void) 
{
    ...
    Comp_Reset_PWRMGR &= (uint8)(~Comp_Reset_ACT_PWR_EN);
    Comp_Reset_STBY_PWRMGR &= (uint8)(~Comp_Reset_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_Reset_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_Reset_lowPowerBackup.compCRReg = Comp_Reset_CR;
        Comp_Reset_CR = Comp_Reset_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_Reset_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_Reset_SetSpeed(uint8 speed) 
{
    ...
    Comp_Reset_CR = (Comp_Reset_CR & (uint8)(~Comp_Reset_PWR_MODE_MASK)) |
                           (speed & Comp_Reset_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_Reset_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Reset_TR = Comp_Reset_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Reset_TR0 = Comp_Reset_HS_TRIM_TR0;
            Comp_Reset_TR1 = Comp_Reset_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Reset_TR = Comp_Reset_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Reset_TR0 = Comp_Reset_LS_TRIM_TR0;
            Comp_Reset_TR1 = Comp_Reset_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_Reset_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_Reset_WRK & Comp_Reset_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_Reset_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_Reset_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_Reset_WRK & Comp_Reset_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Reset_TR |= Comp_Reset_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Reset_TR0 |= Comp_Reset_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Reset_TR |= Comp_Reset_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Reset_TR1 |= Comp_Reset_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_Reset_WRK & Comp_Reset_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Comp_Reset_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Comp_Reset_ZeroCal(void) 
{
    ...
    tmpSW0 = Comp_Reset_SW0;
    tmpSW2 = Comp_Reset_SW2;
    tmpSW3 = Comp_Reset_SW3;

     /* Clear routing for inP, retain routing for inN */
    Comp_Reset_SW0 = 0x00u;
    Comp_Reset_SW2 = 0x00u;
    Comp_Reset_SW3 = tmpSW3 & (uint8)(~Comp_Reset_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_Reset_CR;
    Comp_Reset_CR |= (Comp_Reset_CAL_ON | Comp_Reset_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_Reset_TR = Comp_Reset_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_Reset_TR0 = Comp_Reset_DEFAULT_CMP_TRIM;
        Comp_Reset_TR1 = Comp_Reset_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_Reset_CR & Comp_Reset_PWR_MODE_MASK) == Comp_Reset_PWR_MODE_FAST)
    {
        Comp_Reset_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_Reset_trimAdjust(1u);
		Comp_Reset_trimAdjust(0u);
    }
    ...
    Comp_Reset_SW0 = tmpSW0;
    Comp_Reset_SW2 = tmpSW2;
    Comp_Reset_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_Reset_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_Reset_TR1 << 8u) | (Comp_Reset_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_Reset_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_Reset_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_Reset_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_Reset_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
